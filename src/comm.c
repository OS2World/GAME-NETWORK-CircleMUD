...
#ifdef OS2
   if (sock_init()==1) {
     printf("CircleMUD requires IBM TCP/IP 2.0 to operate\n");
     exit(1);
   }
#endif
...
#else
   gtod(&last_time, (struct timezone *) 0);
#endif
...
#else
   thisround = 80;
   while ((total-sofar)>=thisround) {
     if (send(desc, txt+sofar, thisround, 0) < 0) {
       perror("Write to socket");
       return(-1);
     }
     sofar += thisround;
   } ;
   if (send(desc, txt+sofar, total - sofar, 0) < 0) {
     perror("Write to socket");
     return(-1);
   }
...
{
  int blockmode;
  if (ioctl(s, FIONBIO, (unsigned char *)&blockmode, sizeof(blockmode)) == -1 ) {
      perror("Fatal error executing nonblock (comm.c)");
      exit(1);
   }
}
...
int gtod(struct timeval *tv, struct timezone *tz)
{
   ...
   t-=(double)tv->tv_sec* (double) CLOCKS_PER_SEC;
   t/=CLOCKS_PER_SEC;
   t*=1000000;
   tv->tv_usec=0;
}
